...
def norm(xx, mima=False, large= 1e8):  # xx - 2D
    def lw(x): return list(np.where(x))[0]
    
    #pr(Y, f'in norm():  {xx.shape=}' )
    #xx.tab(f'{B}xx', nr=1)
    ...
    if not mima:
        ...
        # nmi, nma= np.where(np.isnan(mi)), np.where(np.isnan(ma))  #mi,ma= mima
        # neq= np.where( np.equal(mi,ma))
        ix_bad= list(set(np.concatenate([lw(np.isnan(mi)), lw(np.isnan(ma)), 
                                         lw(np.equal(mi,ma))]))) 
        x_bad= xx.columns[ix_bad]
    else: mi,ma= mima
    
    pred(f'mi==ma for {len(xx)=}:', np.where(ma==mi)) # [38, 39]
    return PDF((r - mi) / (.001 + ma - mi), index=xx.index, columns=xx.columns),  \
        (mi,ma), x_bad    
...
def calc_yx(h1, mima=False, cy=[], cx2=[], cx_not_imp=[], 
    ...
    u, (mi,ma), x_bad= norm(xx= df1[cx3], mima=mima)
    cx4=  list(set([c for c in cx3 if  c not in x_bad]))
    ...
    yx= conc([h1[['ts','close']], df1[cy], u[cx4]], **a1)
...
def ta_opt_optim():
    ...
    def get_curve_hist(f):
        with open(f) as a:  return PDF(json.loads(a.read())['data'])
      
    hii= [get_curve_hist(f'in/getHistory-{i}.json')  for i in range(1, 6)]  #12)]  
    ...
    if 0:
        ...
        #models= pilo('out/models/m_gbm_3q.pck')   # high perform ?? zzzz
...
class Anal_2freq:
    ...
    def get_data():
        hii= [get_curve_hist(f'in/getHistory-{i}.json')  for i in range(1, 14)]  #12)]  
    ...
    def add_columns_btc_min(u, rmNA=True, toScale=False,  mima=False, 
                            opts=False, opts_taz=False, with_taz=1): 
        ''' u - hi '''
        if 'vol' in u.columns: u= u.rename(columns={'vol':'volume'})
        u['date']= pd.to_datetime(u.ts, unit='s')
        u['hm']= u.ts.apply(lambda t: t % ( 24*3600) / ( 24*3600))  # float time in days
        ...
        def incre(a,b): return np.log((1e-8 + a) /(1e-8 + b))  # log increment
        def em(x,k): return x.ewm(com=k).mean()            # exp moving average
    
        
    
        # no scale for BTC
        u_sc= u.drop(columns=['ts']).copy()    
        r, cl= u_sc, u_sc.close
        
        r['y_1']= incre(cl.shift(-1), cl)  * 200  # % daily
        r['y_2']= incre(cl.shift(-2), cl) / 2 * 200
        r['y_3']= incre(cl.shift(-3), cl) / 3 * 200
        ...
        #opts_taz= opts_taz or [2,  6, 2,  4, 3,   2, 4, 6,  8]
        #w_fast,w_slow,w_sign,w_,w_sm,  w1,w2,w3,w4 = opts_taz 
        if with_taz==1: w_fast,w_slow,w_ = opts_taz
        if with_taz==2: w_fast,w_slow,w_, w_fast2,w_slow2,w_2 = opts_taz
        
        
        high, low, close= u_sc.high, u_sc.low, u_sc.close
        
        if 0: #with_taz > 0:  #  https://technical-analysis-library-in-python.readthedocs.io/en/latest/ta.html#volatility-indicators
            w_fast,w_slow,w_ = [2,14,2]
            u_sc['taz_ma_in']= ta.trend.mass_index(high, low, 
                                            window_fast=w_fast, window_slow=w_slow)
            u_sc['taz_atr']= ta.volatility.average_true_range(high, low, close, window=w_)
            u_sc['taz_adx']= ta.trend.adx(high, low, close, window=w_)
            
        if 0:  #with_taz > 1:  #  https://technical-analysis-library-in-python.readthedocs.io/en/latest/ta.html#volatility-indicators
            w_fast2,w_slow2,w_2 = [3,11,3]  #[3,15,8]  # [3, 10, 3]
            u_sc['taz_ma_in2']= ta.trend.mass_index(high, low, 
                                           window_fast=w_fast2, window_slow=w_slow2)
            u_sc['taz_atr2']= ta.volatility.average_true_range(high, low, close, window=w_2)
            u_sc['taz_adx2']= ta.trend.adx(high, low, close, window=w_2)
           
            
        if with_taz == 3:
            pr(f'{opts_taz=}')
            df= u_sc.dropna().copy()
            pr(B, f'{opts_taz=}, {df.shape=}, {df.columns=}')

            for i in [0,1]:
                #w1, w2, w3, w4= opts_taz[i]
                w_fast, w_slow, w_sign, w_,w_sm,   w1,w2,w3,w4 = ot= opts_taz[i][:] 
                if 0:
                    u_sc[f'taz_ma_in_{i}']= ta.trend.mass_index(high, low, 
                                                    window_fast=w_fast, window_slow=w_slow)
                    u_sc[f'taz_atr_{i}']= ta.volatility.average_true_range(high, low, close, window=w_)
                    u_sc[f'taz_adx_{i}']= ta.trend.adx(high, low, close, window=w_)
                    
    
                    u_sc[f'taz_kst_{i}']= ta.trend.kst(close, roc1=5, roc2= 7, roc3= 10, roc4= 15, 
                                            window1=w1, window2=w2, window3=w3, window4=w4)
                    u_sc[f'taz_ichimoku_b_{i}']= ta.trend.ichimoku_b(high, low, window2=w2, window3=w3)
                    u_sc[f'taz_kcw_{i}']= ta.volatility.keltner_channel_wband(high, low, close, 
                                                          window=w_, window_atr=w_sign)   
                import warnings
                warnings.filterwarnings("ignore", category=Warning)
                pr(Y, f'\nin add_columns_btc_min():  {ot=}')
                df = add_all_ta_features(df, open="open", high="high", low="low", 
                       close="close",  volume="volume", fillna=False, 
                       colprefix=f'ta_{i}_', opts=ot)
                warnings.resetwarnings()
            ...
            x_d1_not_imp=[]
            cxy= [x for x in df.columns  if x[0] !='y' and  x not in ['date'] + x_d1_not_imp ]
            
            df= conc([df, 
                      PDF({x+f'_d1': dt1(df[x]) for x in cxy }),                           
                      PDF({x+f'_d2': dt2(df[x]) for x in cxy}), 
                     ] , **a1)
        ...
        cx= [c for c in u_sc.columns if c[0] != 'y' and c !='date']         
        cy= [c for c in u_sc.columns if c[0] == 'y']
        
        
        #pr(G, f'{u_sc[cx]=}')

        nx, mima, cx_bad= norm(u_sc[cx], mima=mima, large= 1e8) 
        
        df=  conc([u.date, u_sc[cy], nx] , **a1)        
        df.cx, df.cy, df.cx_bad= cx, cy, cx_bad
        #pr(Y, f'{df.cx_bad=}')
        #if len(x_bad) > 0: 
...
def win(trial, oo=False, with_taz= 3) :  #, cx_bad=cx_bad): #, hii=hii):
    cx_bad= gl.cx_bad
    if oo:
        opts_taz= oo.values()
    else: 
        if with_taz== 3:
            opts_taz= [0,1]
            for i in [0,1]:
                w_fast= trial.suggest_int(f'w_fast_{i}', 2, 3)
                w_slow= trial.suggest_int(f'w_slow_{i}', w_fast+7, w_fast+12)
                w_sign= trial.suggest_int(f'w_sign_{i}', 2, 6)
                w_= trial.suggest_int(f'w_{i}', 2,14)
                w_sm= trial.suggest_int(f'w_{i}', 2,14)
                
                w1=  trial.suggest_int(f'w1_{i}', 1, 3)
                w2=  trial.suggest_int(f'w2_{i}', w1+1, w1+4)
                w3=  trial.suggest_int(f'w3_{i}', w2+1, w2+4)
                w4=  trial.suggest_int(f'w4_{i}', w3+1, w3+4)
                opts_taz[i] = [w_fast, w_slow, w_sign, w_,w_sm,   w1,w2,w3,w4]
        else:        
            w_fast= trial.suggest_int('w_fast', 2, 3) # 7)
            w_slow= trial.suggest_int('w_slow', w_fast+7, w_fast+12) #, w_fast+1, w_fast+12)
            #w_sign= trial.suggest_int('w_sign', 2, 4) #9)
            w_= trial.suggest_int('w_', 2,14)    #2, 12)]
            if 0:
                w_fast2= trial.suggest_int('w_fast2', 2, 3) # 7)
                w_slow2= trial.suggest_int('w_slow2', w_fast+7, w_fast+12) #, w_fast+1, w_fast+12)
                w_2= trial.suggest_int('w_2', 2,14)    #2, 12)]
            
            opts_taz= [w_fast, w_slow, w_]
        #opts_taz= [w_fast, w_slow, w_ ,   w_fast2, w_slow2, w_2]   
    
    if 0: # initial for  mima, cx, cy
        yx0, mima= Anal_2freq.add_columns_btc_min(hii[0], rmNA=True, toScale=False, 
                                             mima=False, opts=False, 
                                             opts_taz=[[1]*8,[1]*8], with_taz=3)
        yx0.tab('yx0');
        yx0.taz_ma_in_0.vc(dropna=0)  
        cx, cy = yx0.cx, yx0.cy
        pidu((mima, cx, cy), 'out/mima_827.pck')
        
    mima, cx, cy= pilo('out/mima_827.pck')    
    
    yxx= [Anal_2freq.add_columns_btc_min(h, rmNA=True, toScale=False, 
                    mima=mima, opts=False, opts_taz=opts_taz, with_taz= with_taz)[0] for h in hii[-4:]]
    #pr(Y, f'{len(yxx[:-1])=}, \n{yxx[:-1][0]=}')
    cxy_good= [c for c in (cx+cy) if c not in cx_bad]
    tr, te= conc(yxx[:-1])[cxy_good], yxx[-1]
    nans= np.sum(np.isnan(tr))
    
    pr(Y, f'in win():  {len(cx_bad)=} {len(gl.cx_bad)=} {gl.cx_bad=}')
    plt.imshow(tr)

    
    def q(x): return w(x >0, 1, -1)
    def q3(x, thr=.93):              # <----------
        v= x[:,1]- x[:,0]
        return w(v >thr, 1, w(v < -thr, -1, 0))
    
    cx_good= [c for c in cx if c not in gl.cx_bad]
    pr(G, f'in win():  {len(cx_good)=}  \n{cx_good=}, \n{tr[cx_good]=}')

    
    u= tr[cx_good]  
    pr(B, u.shape)
    pr(B, np.isnan(u))
    pr(Y, np.nansum(np.isnan(u), axis=0))
    plt.imshow(u)
    
    nn=  np.nansum(np.isnan(u), axis=0)  # 0
    cx_nn=  u.columns.astype(str)[nn >100] # 0
    cx_bad2= sorted( list(cx_bad) + list(cx_nn)); len(cx_bad2)
    pr(B, f'{cx_nn=},  \n{cx_bad2=}, \n{len(cx_bad2)=}, ')
    
    c_good2=  sorted(list(set( [c for c in (cx_good + cy) if c not in cx_bad2])))
    cx_good2= [c for c in c_good2 if c[0] != 'y']
    
    tr, te= tr[c_good2].dropna(), te[c_good2].dropna()
   
    pr(Y, f'\n{tr.shape=}, {tr[c_good2].shape=}\n')
    

    models3= {}
    for  y in ['y_1']:  #, 'y_2', 'y_3']:  #['y_10']: # cy: # ['y_1', 'y_2', 'y_3', 'yq_1', 'yq_2', 'yq_3']
        pr(y)
        m= lgb.LGBMClassifier(objective="binary",  **sbm).fit(tr[cx_good2], q(tr[y].values))
        m.cx= cx
        m.sbm= sbm
        m.cy= cy
        models3[y]= m
    
    #yh= m.predict_proba(te[cx_good2])
    #pr(B, f'{yh=}')
    #pr(G, f'{tr.shape=}, {te.shape=},  \n\n{q(yh)[:11]=}')
    #te.tab('te');
    
    yhh= PDFd(y= q(te[y]),
              #yh= q3(m.predict_proba(te[cx])[:, 1] )     
              yh= q3(m.predict_proba(te[cx_good2]) )     
            )
    ...
    conf= yhh.vc()
    pr('\n',  B, '\n', PDF(conf, columns=['n']).ri(). pivot(index='y', columns='yh', values='n'), W)
...
if 0: 
    ...
    #### Start here ------------------------------------------------------------
    yx0, mima= Anal_2freq.add_columns_btc_min(hii[0], rmNA=True, toScale=False, 
                        #mima=False, opts=False, opts_taz=[[1]*9]*2, with_taz=3)
                        mima=False, opts=False, opts_taz=[[1,2,3,4,5,6,7,8,9]]*2, with_taz=3)
    yx0[300:302].tab('yx0',nr=1);
    #yx0.taz_ma_in_0.vc(dropna=0)  
    cx, cy, cx_bad = yx0.cx, yx0.cy, yx0.cx_bad
    # pidu((mima, cx, cy), 'out/mima_827.pck')

    if 1:
     nn= sum( np.isnan(yx0))  # 0
     cx_nn=  yx0.columns.astype(str)[nn >20] # 0
     cx_bad= sorted( list(cx_bad) + list(cx_nn)); len(cx_bad)
     
    
    gl.cx_bad= cx_bad
     
    
    sum(np.isnan(yx0[cx_nn]))  # 0
     

if 0:
    w= win(0, oo={'w_fast': 2, 'w_slow': 11, 'w_': 4, 
           'w_fast2': 3, 'w_slow2': 10, 'w_2': 3},)
    w

if 0:
    study = optuna.create_study(direction='maximize')
    study.optimize(win, n_trials= 99)
    study.best_params, study.best_value
    
    # all ta with diff, thr= .92
    {'w_fast_0': 3, 'w_slow_0': 12,'w_sign_0': 3,'w_0': 4,
      'w1_0': 3,    'w2_0': 6, 'w3_0': 9, 'w4_0': 12,
      'w_fast_1': 2,
      'w_slow_1': 13,
      'w_sign_1': 4,
      'w_1': 13,
      'w1_1': 3, 'w2_1': 7, w3_1': 11, 'w4_1': 13}, 1.199
     
    {'w_fast_0': 3,  'w_slow_0': 15, 'w_sign_0': 3,  'w_0': 3,
      'w1_0': 1,     'w2_0': 5,      'w3_0': 8,      'w4_0': 12,
      'w_fast_1': 2, 'w_slow_1': 9,  'w_sign_1': 2,  'w_1': 14,
      'w1_1': 3,     'w2_1': 7,      'w3_1': 11,     'w4_1': 13},  0.1999999999999993
    
    {'w_fast': 2, 'w_slow': 11, 'w_': 4}, -25.0 
    
    {'w_fast': 2, 'w_slow': 11, 'w_': 4, 
     'w_fast2': 3, 'w_slow2': 10, 'w_2': 3}, 0.599
    
    # with fixt set 2
    {'w_fast': 3, 'w_slow': 15, 'w_': 8}, -15.40
    {'w_fast': 3, 'w_slow': 11, 'w_': 3}, -10.600
